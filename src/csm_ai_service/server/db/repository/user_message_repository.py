...
from csm_ai_service.server.db.models.user_message_model import UserMessageModel
from csm_ai_service.server.db.session import with_session


@with_session
def add_user_message_to_db(
        session,
        chat_type,
        query,
        response="",
...
@with_session
def update_user_message(session, message_id, response: str = None):
    """
    更新已有的聊天记录
    """
    m = get_user_message_by_id(message_id)
    if m is not None:
...
@with_session
def get_user_message_by_id(session, message_id) -> UserMessageModel:
    """
    查询聊天记录
    """
