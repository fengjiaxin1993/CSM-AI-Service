...
from csm_ai_service.server.db.models.knowledge_base_model import KnowledgeBaseModel
from csm_ai_service.server.db.models.knowledge_file_model import (
    FileDocModel,
    KnowledgeFileModel,
)
...
def add_docs_to_db(session, kb_name: str, file_name: str, doc_infos: List[Dict]):
    ...
    if doc_infos is None:
        print(
            "输入的csm_ai_service.server.db.repository.knowledge_file_repository.add_docs_to_db的doc_infos参数为None"
        )
        return False
    for d in doc_infos:
        obj = FileDocModel(
