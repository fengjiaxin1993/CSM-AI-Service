...
from csm_ai_service.server.conversation.file_rag.text_splitter import (
    AliTextSplitter,
...
from csm_ai_service.server.utils import run_in_thread_pool
...
def get_user_vs_path(user_id: str, vector_name: str):
    return os.path.join(get_user_path(user_id), "vector_store", vector_name)
...
# 解析pdf最快 https://www.aidoczh.com/langchain/v0.2/docs/how_to/document_loader_pdf/#%E4%BD%BF%E7%94%A8-pymupdf
...
def get_loader(loader_name: str, file_path: str, loader_kwargs: Dict = None):
    ...
    try:
        if loader_name in _COMMUNITY_LOADERS:
            DocumentLoader = _COMMUNITY_LOADERS[loader_name]
        else:
            DocumentLoader = UnstructuredFileLoader
...
def make_text_splitter(splitter_name, chunk_size, chunk_overlap):
    ...
    # langchain_text_splitters映射
    _LANGCHAIN_SPLITTERS = {
        "RecursiveCharacterTextSplitter": RecursiveCharacterTextSplitter,
        "SpacyTextSplitter": TextSplitter,  # SpacyTextSplitter在langchain_text_splitters中已移除，回退到TextSplitter
    }
    ...
    try:
        if (
            ...
            # 优先使用用户自定义的text_splitter，否则使用langchain的text_splitter
            if splitter_name in _CUSTOM_SPLITTERS:
                TextSplitterClass = _CUSTOM_SPLITTERS[splitter_name]
            elif splitter_name in _LANGCHAIN_SPLITTERS:
                TextSplitterClass = _LANGCHAIN_SPLITTERS[splitter_name]
            else:
                TextSplitterClass = RecursiveCharacterTextSplitter
        ...
        print(e)
        TextSplitterClass = RecursiveCharacterTextSplitter
        text_splitter = TextSplitterClass(chunk_size=chunk_size, chunk_overlap=chunk_overlap)
...
class KnowledgeFile:
    ...
    def file2docs(self, refresh: bool = False):
        if self.docs is None or refresh:
            ...
            if isinstance(loader, TextLoader):
                ...
                self.docs = loader.load()
            logger.info("解析文件【{}】 花费 {:.2f} 秒".format(self.filename, time.time() - time1))
        return self.docs
    ...
    def file2text(
        ...
        if self.splited_docs is None or refresh:
            ...
            logger.info("分割文件【{}】花费 {:.2f} 秒".format(self.filename, time.time() - start_time1))
        return self.splited_docs
