...
from csm_ai_service.server.csm_analyze.warning_analysis.extract_info.helper import _init_structured_fields, bbox_in_area, clean_text


class PDFAlarmReportParser:
    ...
    def __init__(self, pdf_path: str):
        ...
        try:
            ...
            self.tables = self.__get_tables()  # 1.先读取table数据
    ...
    def parse(self) -> Dict:
        ...
        # 2. 提取告警信息
        self.structured_data["告警信息"] = self.__extract_section_content("一、告警信息", "二、处置过程")
        # 提取告警表格信息
...
if __name__ == "__main__":
    # 测试PDF解析（word转换成的PDF）
    PDF_PATH = "../pdf-告警分析报告模板 -demo.pdf"
    parser = PDFAlarmReportParser(PDF_PATH)
    result = parser.parse()
    print("=== 电力告警报告PDF结构化解析结果 ===")
